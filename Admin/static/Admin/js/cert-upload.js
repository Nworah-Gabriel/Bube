...
// Add blur to main content when modal is shown
function addBlurToMainContent() {
    const mainContent = document.querySelector('.mil-content');
    if (mainContent) {
...
// Remove blur from main content when modal is hidden
function removeBlurFromMainContent() {
    const mainContent = document.querySelector('.mil-content');
    if (mainContent) {
...
// Custom Success Notification with Blurry Background
function showSuccessNotification(message) {
    // Remove any existing notifications and backdrops
    const existingNotifications = document.querySelectorAll('.mil-success-notification');
    const existingBackdrops = document.querySelectorAll('.mil-notification-backdrop');
    existingNotifications.forEach(notification => notification.remove());
    existingBackdrops.forEach(backdrop => backdrop.remove());
    
    // Add blur to main content
    addBlurToMainContent();
    
    // Create blurry backdrop
    const backdropHTML = `
        <div class="mil-notification-backdrop" style="
            position: fixed;
            top: 0;
            left: 0;
            width: 100%;
    ...
    style.textContent = `
        ...
        .mil-notification-backdrop {
            animation: fadeIn 0.3s ease;
        }
        
        .mil-success-notification {
            animation: slideIn 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275);
        }
        ...
        #notification-continue:hover > div {
            left: 100%;
        }
        
        #notification-continue:hover svg {
            transform: scale(1.1);
        }
    ...
    // Insert backdrop first, then notification
    ...
    document.querySelector('.mil-notification-backdrop').addEventListener('click', function() {
        hideNotification();
    });
...
function hideNotification() {
    ...
    const backdrop = document.querySelector('.mil-notification-backdrop');
    
    // Remove blur from main content
    removeBlurFromMainContent();
    const mainContent = document.querySelector('.mil-content');
    if (mainContent) {
...
async function uploadCertificate() {
    try {
        ...
        if (result.success) {
            ...
            // Add blur to main content for error modal too
            addBlurToMainContent();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    labels.forEach(label => {
        if (label.getAttribute('for') === 'certTitle' || 
            label.getAttribute('for') === 'certIssuer' || 
            label.getAttribute('for') === 'admCertInput') {
